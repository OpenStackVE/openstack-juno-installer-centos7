...
memzero=`ps --user qemu u|awk '{print $4}'|grep -c -i -v "MEM"`
...
then
	virt-top -1 -d 1 -b -n 4 --script --no-csv-mem --no-csv-block --no-csv-net --csv /tmp/$csvfilename.csv
...
0)
        ...
        ;;
*)
        listqemus=`ps --user qemu u|awk '{print $4}'|grep -i -v "MEM"`
        for i in $listqemus
        do
                # echo $i
